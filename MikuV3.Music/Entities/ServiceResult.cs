...
namespace MikuV3.Music.Entities
{
    public class ServiceResult
    {
        ...
        public BufferedStream PCMCache { get; set; }
        ...
        public ServiceResult(ContentService cs, HttpClient c, TimeSpan l, List<string> du, string u, HttpResponseMessage resp = null, string a = "n/a", string t = "/", bool s = false)
        {
            ContentService = cs;
            Client = c;
            Length = l;
        ...
        public async Task FillCache()
        {
            try
            {
                ...
                if (ResponseMsg == null) ResponseMsg = await Client.GetAsync(DirectUrls[0], HttpCompletionOption.ResponseHeadersRead);
                var psi2 = new ProcessStartInfo()
                {
                    FileName = @"ffmpeg.exe",
                    Arguments = $@"-i - -af ""aresample=async=1:first_pts=0"" -ac 2 -f s16le -ar 48000 pipe:1",
                        ...
                        foreach (var part in DirectUrls)
                        {
                            ...
                            using (var thedata = await Response.Content.ReadAsStreamAsync())
                            {
                                ...
                                while (read != 0)
                                {
                                    ...
                                    if (Slow)
                                    {
                                        while (((read = thedata.Read(buffer, 0, buffer.Length)) > 0) && Percentage > 25)
                                        {
